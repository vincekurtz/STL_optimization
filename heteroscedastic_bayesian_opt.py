...
state_dependent_noise = True  # For testing
...
if state_dependent_noise:
    ...
    error = 0.2+ np.zeros(X.size)[:,None]
Y = f(X) + error
...
for i in range(n_iter):
    ...
    ax[0].errorbar(X,Y,yerr=abs(error.flatten()),fmt="none",ecolor='k',zorder=1)
