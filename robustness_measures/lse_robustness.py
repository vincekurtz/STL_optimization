...
# of STL signals, using the LogSumExp (LSE) approximation of min and max functions.
#
##
...
class STLFormula:
    ...
    def conjunction(self, second_formula):
        ...
        """
        new_robustness = lambda s, t : min_lse( [self.robustness(s,t),
    ...
    def disjunction(self, second_formula):
        ...
        Arguments:
            second_formula : an STL Formula or predicate defined over the same signal s.
        """
        new_robustness = lambda s, t : max_lse( [self.robustness(s,t),
                                            second_formula.robustness(s,t)] )
        new_formula = STLFormula(new_robustness)
    ...
    def eventually(self, t1, t2):
        ...
        """
        new_robustness = lambda s, t : max_lse([ self.robustness(s,k) for k in range(t+t1, t+t2+1)])
    ...
    def always(self, t1, t2):
        ...
        Arguments:
            ...
            t2 : an integer between t1 and signal length T
        """
        new_robustness = lambda s, t : min_lse([ self.robustness(s,k) for k in range(t+t1, t+t2+1)])
